...
# Setup
If you want to use this bot yourself, please feel free to do so but please keep in mind that this was designed for our community.  Most variables that require change can be found within the `update.json` file and the Token can be implemented within the `config.json` file.
...
`check` | Sends a DM to the user containing the latest ticket completion statistics
...
`editmessageid` | Edits a message based on ID.  This will only work if the bot posted this its-self. | `!editmessageid 645839312415358996 Raaaaah.  I'm aliiiive`
...
`elon` | Sends all mighty Elon picture.  Because why not
